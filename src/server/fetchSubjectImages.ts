...
export async function fetchSubjectImages(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(
        `Custom Search API returned ${response.status} for subject "${subject}".${errorBody ? ` Body: ${errorBody}` : ""}`,
      );
      return attemptFallback(subject, normalizedQuery, options);
    }
    ...
    console.error("Failed to fetch subject images", error);
    return attemptFallback(subject, normalizedQuery, options);
  }
}
...
async function fetchFallbackSubjectImages(
  ...
  try {
    ...
    console.error("Fallback person image lookup failed", error);
    return [];
  }
}
...
async function resolvePrimarySubject(
  ...
  try {
    ...
    console.warn("Falling back to raw query for image subject", error);
